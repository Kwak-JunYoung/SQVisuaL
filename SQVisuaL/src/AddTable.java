...
public class AddTable extends JFrame {
   ...
   public AddTable(MainFrame mf, SQVisuaL sql) {
	...
	String column[]={"Column Name","Data Type","Primary Key?", "Can be NULL?", "Must be unique?"};
	   //String data[][]={ {"Name","VARCHAR", "No", "No", "No"}, {"ID","VARCHAR","Yes", "-", "-"}    };
	...
	Create.addActionListener(new ActionListener(){
		public void actionPerformed(ActionEvent e) {
			...
			for (int i = 0; i < rc; i++) {
    			...
    			if(((String) model.getValueAt(i, 2)).equals("Yes")) pkidx.add(i);
    			if(((String) model.getValueAt(i, 4)).equals("Yes")) unique.add(i);
    			if(((String) model.getValueAt(i, 3)).equals("No")) q += " NOT NULL";
    			if(i != rc - 1) q += ", ";
    		}
			for(int i = 0; i < pkidx.size(); i++) {
				pks += "`" + (String) model.getValueAt(i, 0) + "`";
				if(i != pkidx.size() - 1) pks += ", ";
			}
			pks += ")";
			for(int i = 0; i < unique.size(); i++) {
				uni += "`" + (String) model.getValueAt(i, 0) + "`";
				if(i != unique.size() - 1) uni += ", ";
			}
			uni += ")";
			if(pkidx.size() > 0) q += ", " + pks;
			if(unique.size() > 0) q += ", " + uni;
			q += ");";
			SQLException err = sql.getProvider().updateQueryReturnErr(q);
			if(err != null) setErrorMsg(err.getMessage());
			else {
