	...
	public InsertDataFrame(SQVisuaL sql, MainFrame mf) {
		...
		table = new JTable(new DefaultTableModel(columns, 0)) {
			public boolean isCellEditable(int row, int column) {
				switch (column) {
				case 1:
					if (this.getModel().getValueAt(row, 2) == null)
						return true;
					else if (this.getModel().getValueAt(row, 2).equals("Set to NULL"))
						return false;
					return true;
				case 2:
					if (this.getModel().getValueAt(row, 2) == null)
						return true;
					else if (this.getModel().getValueAt(row, 2).equals("Unavailable"))
						return false;
					return true;
				default:
					return false;
		...
		execute.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < rc; i++) {
					...
					if (model.getValueAt(i, 2).equals("Set to NULL"))
						data += "NULL";
					else
			...
			public void itemStateChanged(ItemEvent e) {
				if (e.getStateChange() == ItemEvent.SELECTED) {
					...
					for (MetaRow r : arr) {
						Object[] row;
						if (!r.canBeNull()) {
							row = new Object[] { r.getName(), "", "Unavailable", r.getType(),
